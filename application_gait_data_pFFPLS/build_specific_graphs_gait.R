# APLICACIÓN GAIT (solo caso de 40 bases):
# - IMSE predicción sobre la test para todos los métodos, excepto FFPLS_OB, Y PARA 3 COMPONENTES
# - CVE estimación sobre la training SOLO para los métodos FFPLS y pFFPLS y para un número de componentes variando de 1 a 5
...
input_folder = "results_gait/reps100_pen100_K40L40/"
...
out_folder_specific <- paste0(out_folder, "IMSEs_CVEs_specific/")

if (!dir.exists(out_folder_specific)) {
  dir.create(out_folder_specific)
}
...
all_final_res_1 <- all_final_res %>% 
  filter(method != "FFPLS_OB", nComp == 3)
...
ggsave(p_imse_val_1,
       filename = paste0(out_folder_specific,
                         "test_imseY_3cmp.png"  ),
       width = 8, height = 8 )
...
# Plot 2
# CVE estimación sobre la training SOLO para los métodos FFPLS y pFFPLS y 
# para un número de componentes variando de 1 a 5
...
p_cve_2 <- ggplot(all_cves_2,
                  aes(x = as.factor(nComp), y = CVE, fill = method)) +
  geom_boxplot(position=position_dodge(0.8))  +
  ylab("Training: CVE(Y)") +
  xlab("# of components") +
  scale_fill_manual(values = color_codes)+
...
ggsave(p_cve_2,
       filename = paste0(out_folder_specific,
                         "train_cveY_pls_methods.png"  ) ,
       width = 8, height = 8 )

ggsave(p_cve_2,
       filename = paste0(out_folder_specific,
                         "train_cveY_pls_methods.pdf"  ) ,
       width = 8, height = 8 )
