...
for(rep_num in rep_starts:total_reps)  {
  ...
  # Generate data -----------------------------------------------------------
  cat("    -> generating train and validation\n")
  
  ntrue = nrow(X_orig)
  n = round(ntrue * (1 - validation_percet))
  ...
  # Preprocessing to impute with KNN
  preprocX <- caret::preProcess(X, method = "knnImpute")
  X <- predict(preprocX, X)
  X_val <- predict(preprocX, X_val)
  
  preprocY <- caret::preProcess(Y, method = "knnImpute")
  Y <- predict(preprocY, Y)
